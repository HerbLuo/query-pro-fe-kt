...
class UpdateTest {
    ...
    fun test() {
        ...
        UserQueryPro.selectBy().id.equalsTo(2).run().also { users: List<User> -> assertEquals(users, listOf(User(2, "hb", 10))) }
        expectSqlResult("UPDATE `user` SET `age` = ? WHERE `user`.`id` = ?", listOf(18, 2))
        UserQueryPro.updateSet(User(id = 2, age = 18)).run().also { assert(it) }
        UserQueryPro.selectBy().id.equalsTo(2).run().also { users: List<User> -> assertEquals(users, listOf(User(2, "hb", 18))) }

        UserQueryPro.selectBy().id.equalsTo(3).run().also { users: List<User> -> assertEquals(users, listOf(User(3, "herb", 18))) }
        expectSqlResult("UPDATE `user` SET `age` = ? WHERE `user`.`id` = ?", listOf(19, 3))
        UserQueryPro.updateSet(User(id = 3, age = 19)).run().also { assert(it) }
        UserQueryPro.selectBy().id.equalsTo(3).run().also { users: List<User> -> assertEquals(users, listOf(User(3, "herb", 19))) }

        UserQueryPro.selectBy().id.equalsTo(4).run().also { users: List<User> -> assertEquals(users, listOf(User(4, "l", 18))) }
        ...
        UserQueryPro.selectBy().id.equalsTo(4).run().also { users: List<User> -> assertEquals(users, listOf(User(4, "hb", null))) }

        UserQueryPro.selectBy().id.equalsTo(5).run().also { users: List<User> -> assertEquals(users, listOf(User(5, "l", 18))) }
        expectSqlResult("UPDATE `user` SET `age` = ? WHERE `user`.`id` = ?", listOf(NULL, 5))
        UserQueryPro.updateSet().id(5).age(NULL).run().also { assert(it) }
        UserQueryPro.selectBy().id.equalsTo(5).run().also { users: List<User> -> assertEquals(users, listOf(User(5, "l", null))) }

        UserQueryPro.selectBy().id.equalsTo(6).run().also { users: List<User> -> assertEquals(users, listOf(User(6, "lhb", 18))) }
        UserQueryPro.selectBy().id.equalsTo(7).run().also { users: List<User> -> assertEquals(users, listOf(User(7, "lhb", 20))) }
        ...
        UserQueryPro.selectBy().id.equalsTo(6).run().also { users: List<User> -> assertEquals(users, listOf(User(6, "lhb", 22))) }
        UserQueryPro.selectBy().id.equalsTo(7).run().also { users: List<User> -> assertEquals(users, listOf(User(7, "lhb", 22))) }
    }
}
